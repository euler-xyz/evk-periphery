...
>
> The safe pattern is **transient grant-and-revoke within the migration batch**: include the Aave `delegationWithSig` / Morpho `setAuthorizationWithSig` grant, the `*ForSender` leg, and the matching signature-based revocation in the same EVC batch, so no exploitable grant persists after the batch. Ordinary ERC20/Permit2 allowances for `transferFromSender` are far less dangerous — they only ever move the *granting* caller's own tokens to a `to` that same caller picks — but the same grant-and-revoke hygiene is recommended for unlimited approvals.
