...
contract ClusterDump is ScriptUtils {
    function run() public {
        ...
        for (uint256 i = 0; i < vaultInfo.length; ++i) {
            ...
            line =
                string.concat(line, ",", formatPercentage(irmInfo.interestRateInfo[1].borrows * 1e4 / type(uint32).max));
            line = string.concat(line, ",", formatPercentage(vaultInfo[i].interestFee));
            line = string.concat(line, ",,,,", formatPercentage(irmInfo.interestRateInfo[1].supplyAPY * 1e4 / 1e27));
            line = string.concat(line, ",", formatPercentage(irmInfo.interestRateInfo[1].borrowAPY * 1e4 / 1e27));
            line = string.concat(line, ",", formatPercentage(irmInfo.interestRateInfo[2].borrowAPY * 1e4 / 1e27));
            vm.writeLine(outputScriptFileName, line);
            line = "";
        }
