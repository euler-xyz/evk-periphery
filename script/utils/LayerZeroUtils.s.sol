...
contract LayerZeroUtil is ScriptExtended {
    ...
    function _fetchChainBundle(uint256 chainId) private returns (string memory) {
        ...
        string memory cmd = string.concat(
            ...
            vm.toString(chainId),
            " '",
            "((to_entries | map(select(.value.chainDetails.nativeChainId == $id",
            " and .value.chainDetails.chainType == \"evm\"",
            " and ((.value.deployments // [])",
            " | map(select(.endpointV2 and (.eid|tonumber) >= 30000 and (.eid|tonumber) < 40000))",
            " | length > 0))))",
            " | if length == 1 then .[0]",
            " else error(\"expected exactly 1 EVM chain entry for chain \\(",
            "$id), got \\(length)\") end) as $e",
            " | (($e.value.deployments // [])",
            " | map(select(.endpointV2 and (.eid|tonumber) >= 30000 and (.eid|tonumber) < 40000))",
            " | if length == 1 then .[0]",
