...
contract Swap is ScriptUtils {
    function run() public broadcast returns (address swapper, address swapVerifier) {
        ...
        address ensoAggregator = abi.decode(vm.parseJson(json, ".ensoAggregator"), (address));

        (swapper, swapVerifier) =
            execute(oneInchAggregator, uniswapRouterV2, uniswapRouterV3, uniswapRouter02, ensoAggregator);
    ...
    function execute(
        ...
        swapper =
            address(new Swapper(oneInchAggregator, uniswapRouterV2, uniswapRouterV3, uniswapRouter02, ensoAggregator));
