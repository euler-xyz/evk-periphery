...
contract Cluster is ManageCluster {
    function defineCluster() internal override {
        ...
        cluster.clusterAddressesPath = "/script/production/plasma/clusters/M11Reservoir.json";
    ...
    function configureCluster() internal override {
        ...
        cluster.oracleProviders[wsrUSD] = "";
        cluster.oracleProviders[rUSD] = "";
        cluster.oracleProviders[USDT0] = "0xE8947CFd3f04E686741F7Dd9023ec0C78588fd33";

        // define supply caps here. 0 means no supply can occur, type(uint256).max means no cap defined hence max amount
        cluster.supplyCaps[wsrUSD] = type(uint256).max;
        cluster.supplyCaps[rUSD] = type(uint256).max;
        cluster.supplyCaps[USDT0] = type(uint256).max;

        // define borrow caps here. 0 means no borrow can occur, type(uint256).max means no cap defined hence max amount
        cluster.borrowCaps[wsrUSD] = type(uint256).max;
        cluster.borrowCaps[rUSD] = type(uint256).max;
        ...
        //                0         1       2
        //                wsrUSD    rUSD    USDT0
        /* 0  wsrUSD  */ [LTV_ZERO, LTV_HIGH, LTV__LOW],
        /* 1  rUSD    */ [LTV_ZERO, LTV_ZERO, LTV__LOW],
