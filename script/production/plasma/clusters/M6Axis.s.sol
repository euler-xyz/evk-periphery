...
contract Cluster is ManageCluster {
    function defineCluster() internal override {
        ...
        cluster.clusterAddressesPath = "/script/production/plasma/clusters/M6Axis.json";
    ...
    function configureCluster() internal override {
        ...
        cluster.oracleProviders[sxyUSD] = "";
        cluster.oracleProviders[xyUSD] = "";
        cluster.oracleProviders[USDT0] = "0x3541a516288f04bA8eea256B9cF32388F1733C83";

        // define supply caps here. 0 means no supply can occur, type(uint256).max means no cap defined hence max amount
        cluster.supplyCaps[sxyUSD] = type(uint256).max;
        cluster.supplyCaps[xyUSD] = type(uint256).max;
        cluster.supplyCaps[USDT0] = type(uint256).max;

        // define borrow caps here. 0 means no borrow can occur, type(uint256).max means no cap defined hence max amount
        cluster.borrowCaps[sxyUSD] = type(uint256).max;
        cluster.borrowCaps[xyUSD] = type(uint256).max;
        cluster.borrowCaps[USDT0] = type(uint256).max;

        // define IRM classes here and assign them to the assets
        {
            // sxyUSD - no IRM parameters (not borrowing against itself)
            ...
            cluster.kinkIRMParams[xyUSD] = irmUSD_YIELD_RISKIER;
        ...
        //                0               1       2
        //                sxyUSD          xyUSD   USDT0
        /* 0  sxyUSD  */ [uint16(0.00e4), 0.90e4, 0.88e4],
        /* 1  xyUSD   */ [uint16(0.00e4), 0.00e4, 0.88e4],
        /* 2  USDT0   */ [uint16(0.00e4), 0.95e4, 0.00e4]
        ];
