...
contract Cluster is ManageCluster {
    function defineCluster() internal override {
        ...
        cluster.clusterAddressesPath = "/script/production/berachain/clusters/OrigamiCluster.json";
    ...
    function configureCluster() internal override {
        ...
        // of a string.
        cluster.oracleProviders[WBERA ] = "0x0000000000000000000000000000000000000000";
        cluster.oracleProviders[oriBGT] = "ExternalVault|0x8eE9244458285cf09392Ad2B8f7C0479c2413934";
        
        // define supply caps here. 0 means no supply can occur, type(uint256).max means no cap defined hence max amount
        cluster.supplyCaps[WBERA ] = type(uint256).max;
        cluster.supplyCaps[oriBGT] = type(uint256).max;

        // define borrow caps here. 0 means no borrow can occur, type(uint256).max means no cap defined hence max amount
        cluster.borrowCaps[WBERA ] = type(uint256).max;
        cluster.borrowCaps[oriBGT] = type(uint256).max;

        // define IRM classes here and assign them to the assets
        cluster.irms[0] = 0xBF3F39e6A6c65cBc702977Ba2043a160707568ad;

        // define the ramp duration to be used, in case the liquidation LTVs have to be ramped down
        cluster.rampDuration = 1 days;

        // define the spread between borrow and liquidation ltv
        cluster.spreadLTV = 0.05e4;

        // define ltv values here. columns are liability vaults, rows are collateral vaults
        cluster.ltvs = [
            //               0               1     
            //               WBERA           oriBGT  
            /* 0  WBERA  */ [uint16(0.00e4), 0.00e4],
            /* 1  oriBGT */ [uint16(0.80e4), 0.00e4]
        ];
