...
contract Cluster is ManageCluster {
    function defineCluster() internal override {
        ...
        cluster.clusterAddressesPath = "/script/production/monad/clusters/3p/sMON-WMON-AUSD.json";
    ...
    function configureCluster() internal override {
        ...
        cluster.oracleProviders[sMON] = "";
        cluster.oracleProviders[WMON] = "";
        cluster.oracleProviders[AUSD] = "";

        // define supply caps here. 0 means no supply can occur, type(uint256).max means no cap defined hence max amount
        cluster.supplyCaps[sMON] = 240_000_000;
        cluster.supplyCaps[WMON] = 216_000_000;
        cluster.supplyCaps[AUSD] = 21_600_000;

        // define borrow caps here. 0 means no borrow can occur, type(uint256).max means no cap defined hence max amount
        cluster.borrowCaps[sMON] = 144_000_000;
        cluster.borrowCaps[WMON] = 194_000_000;
